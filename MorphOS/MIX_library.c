...
STATIC CONST TEXT libname[] = "sdl2_mixerX.library";

struct ExecBase   *SysBase  = NULL;
struct DosLibrary *DOSBase  = NULL;
struct Library    *SDL2Base = NULL;
struct Library    *VorbisFileBase = NULL;
struct Library    *SDL2MixerBaseX = NULL;
//struct Library 	  *MPG123Base = NULL;
...
static int init_libs(struct SDL2MixerLibrary *base, struct ExecBase *SysBase)
{
...
struct Library	*LIB_Init(struct SDL2MixerLibrary *LibBase, BPTR SegList, struct ExecBase *sysBase)
{
	register char *r13;
...
static BPTR DeleteLib(struct SDL2MixerLibrary *LibBase, struct ExecBase *SysBase)
{
	BPTR SegList = 0;
...
{
	struct SDL2MixerLibrary *LibBase = (APTR)REG_A6;
...
BPTR LIB_Close(void)
{
	...
	if (LibBase->Parent)
	{
		struct SDL2MixerLibrary *ChildBase = LibBase;
...
{
	struct SDL2MixerLibrary *LibBase = (APTR)REG_A6;
	struct SDL2MixerLibrary	*newbase, *childbase;
	struct ExecBase *SysBase = LibBase->MySysBase;
	...
	ForeachNode(&LibBase->TaskContext.TaskList, ChildNode)
	{
		...
		{
			/* Yep, return it */
			childbase = (APTR)(((ULONG)ChildNode) - offsetof(struct SDL2MixerLibrary, TaskContext.TaskNode.Node));
	...
	if (LibBase->Alloc == 0)
	{
		...
		// && ((MPG123Base 	   =   OpenLibrary("mpg123.library"		,  53)) != NULL)
		)
		{
			LibBase->Alloc = 1;
		}
...
{
	sizeof(struct SDL2MixerLibrary),
	(size_t)FuncTable,
	0,
	(size_t)LIB_Init
};
